...
- Be complete but concise (400-600 words of content)
...
Company: Acme Manufacturing LLC | CAGE: 3KXB4 | ISO 9001:2015 Cert QM-44912 | IATF 16949 Cert IT-88201 | Authorized Signatory: James Martinez, Quality Manager`;
  ...
  async function sendMessage(textOverride) {
    ...
    // Detect doc type from message content
    const lower = text.toLowerCase();
    let docId = activeDoc;
    if (lower.includes("psw") || lower.includes("warrant") || lower.includes("ppap element")) docId = "psw";
    else if (lower.includes("8d") || lower.includes("corrective action") || lower.includes("ncr-00")) docId = "8d";
    else if (lower.includes("coa") || lower.includes("certificate of analysis") || lower.includes("test result")) docId = "coa";
    else if (lower.includes("coo") || lower.includes("certificate of origin")) docId = "coo";
    else if (lower.includes("fair") || lower.includes("first article")) docId = "fair";
    else if (lower.includes("pfmea") || lower.includes("fmea") || lower.includes("process fmea")) docId = "pfmea";
    ...
    try {
      const resultText = await callClaude(text);
      // Replace placeholder with real result
      setMessages(prev => prev.map(m =>
        m.msgId === msgId ? { ...m, type:"doc", content: resultText } : m
      ));
    } catch (err) {
      console.error("AI call failed:", err);
      // Show the error message, then fall back to local template
      const fallback = getFallback(docId);
      setMessages(prev => prev.map(m =>
        m.msgId === msgId ? { ...m, type:"doc", content: fallback, isLocal: true } : m
      ));
    }
            ...
            <div style={S.chatSide}>
              ...
              <div ref={msgsRef} style={S.chatMsgs}>
                {messages.map((msg, i) => {
                  ...
                  if (msg.type === "streaming" || msg.type === "thinking" || msg.type === "doc") return (
                    <ChatMsg key={i} role="ai">
                      {(msg.type === "thinking" && !msg.content) ? (
                        <div style={{ display:"flex", gap:4, alignItems:"center", padding:"4px 0" }}>
                          <span style={{ fontSize:12, color:"rgba(255,255,255,.4)", marginRight:6 }}>Generating document…</span>
                        ...
                        <>
                          {msg.isLocal && (
                            <div style={{ fontSize:11, color:C.amber, marginBottom:6, padding:"5px 9px", background:"rgba(245,166,35,.1)", borderRadius:6, border:"1px solid rgba(245,166,35,.2)" }}>
                              ⚠ AI unavailable — showing local template. Check your API key in Vercel environment variables.
                            </div>
                          )}
                          <DocOutput docId={msg.docId || activeDoc} content={msg.content} isStreaming={false} />
                        </>
                      ) : (
                        <div style={{ display:"flex", gap:4, alignItems:"center", padding:"4px 0" }}>
                          {[0,.2,.4].map((d,k)=><span key={k} style={{ width:5,height:5,borderRadius:"50%",background:C.sky,opacity:.35,animation:`blink 1.2s ${d}s infinite` }} />)}
